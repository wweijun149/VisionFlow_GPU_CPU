...
class BatchInspectionProcessor(LogMixin):
    ...
    def _resolve_gc_interval() -> int:
        """Images processed between gc.collect(0) sweeps (0 disables it)."""
    ...
    def _worker_count(self, image_count: int) -> int:
        ...
        # Scale with available cores instead of a fixed cap of 4; oversubscription
        # against OpenCV's own thread pool is handled by _opencv_thread_budget.
        return max(1, min(8, cpu_count, image_count))

    @contextlib.contextmanager
    def _opencv_thread_budget(self, worker_count: int):
        """Split cores between batch workers and OpenCV's internal pool.

        Each worker runs a full CPU pipeline that itself parallelises via OpenCV.
        With N workers each spawning cpu_count OpenCV threads the process would
        heavily oversubscribe the CPU, so cap OpenCV threads to roughly an even
        share for the duration of the batch and restore the prior setting after.
        """
        if worker_count <= 1:
            yield
            return
