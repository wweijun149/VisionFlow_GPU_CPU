...
- [ ] 🔒實物樣本 五個 production recipes 通過 CPU/GPU 等價規則，沒有未解釋的 fallback。
...
- [ ] 🔒實物樣本 GPU 純檢測 median 與 P95 在目標資料集均優於 CPU；目標加速門檻為至少 1.5 倍。
...
- [x] 2026-09-17：新增 `🔒實物樣本` 標記與圖例，標註需要真實產線 PASS/NG 照片才能完成、且在無實物照片的雲端環境永久無法驗收的項目：production 測試集 baseline、真實 AOI 影像測例、五 recipes PASS/NG 樣本與其等價比較、五 production recipes 逐一 PASS/NG 一致。此標記與 RTX 3090 GPU 編譯／實測項目明確區分——後者可在具 nvcc/GPU 的機器上驗證，並非永久受阻。另依決策將最終驗收門檻的「五 production recipes CPU/GPU 等價」與「GPU 純檢測 median/P95 ≥1.5× CPU」兩項也納入標記，視為須以真實產線資料驗收。
